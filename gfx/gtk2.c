...
static gboolean gtk_expose_callback(GtkWidget *widget, GdkEventExpose *event, gpointer data);
...
window_t *window_create(const char *title, int32_t width, int32_t height) {
    ...
    ctx = malloc(sizeof(gtk_window_context_t));
    if (!ctx) {
        free(window);
        return NULL;
    }
    ...
    g_signal_connect(ctx->drawing_area, "expose-event", G_CALLBACK(gtk_expose_callback), ctx);
...
void window_destroy(window_t *window) {
    ...
    ctx = (gtk_window_context_t*)window->handle;
    if (ctx) {
...
renderer_t *renderer_create(window_t *window) {
    renderer_t *renderer;
    gtk_renderer_context_t *ctx;

    renderer = malloc(sizeof(renderer_t));
    if (!renderer) return NULL;

    ctx = malloc(sizeof(gtk_renderer_context_t));
    if (!ctx) {
        free(renderer);
        return NULL;
    }
    ...
    ctx->current_color.r = 255;
    ctx->current_color.g = 255;
    ctx->current_color.b = 255;
    ctx->current_color.a = 255;
...
void renderer_destroy(renderer_t *renderer) {
    ...
    ctx = (gtk_renderer_context_t*)renderer->handle;
    if (ctx) {
        free(ctx);
    }
    free(renderer);
...
font_t *font_create(const char *path, int32_t size) {
    font_t *font;
    gtk_font_context_t *ctx;

    font = malloc(sizeof(font_t));
    if (!font) return NULL;

    ctx = malloc(sizeof(gtk_font_context_t));
    if (!ctx) {
        free(font);
        return NULL;
    }
...
void font_destroy(font_t *font) {
    ...
    ctx = (gtk_font_context_t*)font->handle;
    if (ctx) {
        if (ctx->layout) {
...
void font_draw_text(renderer_t *renderer, font_t *font, color_t color,
    ...
    gtk_renderer_context_t *rctx;
    gtk_font_context_t *fctx;

    if (!renderer || !font || !text) return;

    rctx = (gtk_renderer_context_t*)renderer->handle;
    fctx = (gtk_font_context_t*)font->handle;
